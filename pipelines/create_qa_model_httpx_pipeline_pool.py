...
class QAPipeline:
    ...
    async def create_qa_batch_async(
        ...
        print(
            msg_info(
                "Generating QA with pipeline-pool async httpx pipeline: "
        ...
        async def _worker(worker_id: int) -> None:
            ...
            while True:
                ...
                job = _build_initial_job(item_id)
                ...
                try:
                    while job.step <= NUM_STEPS:
                        ...
                        print(
                            msg_debug(
                                f"Job start worker_id={worker_id} item_id={job.item_id} "
                        ...
                        print(
                            msg_info(
                                f"Job complete worker_id={worker_id} item_id={job.item_id} "
                                f"step={job.step} elapsed={elapsed:.2f}s "
                    ...
                    await _emit_result(
                        job.item_id,
                        self._final_result(job.item_id, job.previous_outputs),
                    )
                    ...
                    print(
                        msg_error(
                            f"Job failed worker_id={worker_id} item_id={job.item_id} step={job.step} "
