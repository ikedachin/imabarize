"""Temporary uploader that fills missing/null/blank qa_id values with UUIDv4.

CLI arguments are identical to main_upload_dataset.py. Existing qa_id values
and source files are preserved. Explicit exclude-key options still apply.
New UUIDs are generated on every run, independently of the split seed.
...
def _normalize_upload_record(record: dict, exclude_upload_keys: Iterable[str]) -> dict:
    ...
    qa_id = upload_record.get("qa_id")
    if qa_id is None or (isinstance(qa_id, str) and not qa_id.strip()):
        upload_record["qa_id"] = str(uuid.uuid4())
...
"""
uv run python main_upload_dataset_with_uuid.py \
